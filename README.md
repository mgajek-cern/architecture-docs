...
| Authentication Systems | External System | Identity and access management services providing user authentication and authorization through various protocols and credential mechanisms, including federated multi-issuer token services and policy decision points for fine-grained access control |
| Storage Systems | External System | Heterogeneous storage backends including traditional filesystems, object storage, tape archives, cloud storage and external data repositories (such as Copernicus, DestinE, EUCAIM) accessed through standardized protocols, with QoS-aware tiering and HPC parallel filesystems |
...
| Resource Accounting Systems | External System | Credit management and quota enforcement services that track resource consumption, manage allocations, and enforce usage limits across federated infrastructure | 
